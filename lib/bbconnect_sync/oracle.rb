  ...
  class Oracle
    def contacts
      BBConnect::ContactCollection.new.tap do |collection|
        ...
        cursor = connection.exec("SELECT ID, PNAME, LNAME, EMAIL, listagg( AFFILIATION, '|' ) WITHIN GROUP( order by AFFILIATION ) AS AFFILIATIONS, HOUSING_1
                                  FROM BGV_BBCONNECT
                                  GROUP BY ID, PNAME, LNAME, EMAIL, HOUSING_1")
        while row = cursor.fetch_hash
          ...
          collection << BBConnect::Contact.new(id_number, first_name, last_name, email, groups)
        end
      end
    end
