  ...
  class Oracle
    def contacts
      BBConnect::ContactCollection.new.tap do |collection|
        ...
        cursor = connection.exec("SELECT ID, PNAME, LNAME, EMAIL, CELL, listagg( AFFILIATION, '|' ) WITHIN GROUP( order by AFFILIATION ) AS AFFILIATIONS, HOUSING_1
                                  FROM BGV_BBCONNECT
                                  GROUP BY ID, PNAME, LNAME, EMAIL, CELL, HOUSING_1")
